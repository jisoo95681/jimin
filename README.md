...
- `garden.js`: the garden (tree models, growth, locking)
...
- **Garden**: a 3D tree for each study day (three.js, bundled in `vendor/`). Growth follows focused hours: 2h = 25%, 4h = 50%, 6h = 75%, 8h = 100%. The first tree is a Baobab; after that each day gets a random species (never the same two days in a row) from: Baobab, Lemon, Apple, Korean Red Pine, Ginkgo, Grape Vine, Cherry Blossom, Orange, Willow, Maple, Fir, Palm, Oak, Olive, Jacaranda, Birch. When a day ends its tree is locked; no new trees after the exam day. "Concentration garden" shows every tree together.
